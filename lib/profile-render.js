...
function safeJsonLd(value) {
  // JSON.stringify is not enough inside <script type="application/ld+json">:
  // escaped markup like </script> can terminate the script block. Keep the
  // JSON valid while neutralizing HTML/script delimiters from supplier data.
...
function renderProfileBody(supplier, siblings, viewer) {
  ...
  const isDirectoryListing = Number(supplier.directory_listing) === 1;

  // Phone/address visibility: paid profiles always show; outreach-ready
  // directory listings show supplied fields even while marked needs_review.
  const showPhone = isPaid(supplier) || !!supplier.public_phone || isDirectoryListing;
  const showAddress = !!supplier.public_address || isDirectoryListing;

  // Outbound website: paid profiles always show it; outreach-ready free
  // directory listings also show it so businesses can verify/claim.
  const websiteHtml = ((tierAtLeast(supplier.tier, 'pro') || isDirectoryListing) && supplier.website_url)
  ...
  const description = ((tierAtLeast(supplier.tier, 'pro') || isDirectoryListing) && supplier.description)
...
function renderProfileSchemas(supplier) {
  ...
  const localBusiness = {
    ...
    description: (tierAtLeast(supplier.tier, 'pro') || Number(supplier.directory_listing) === 1) && supplier.description ? supplier.description.slice(0, 250) : undefined,
    ...
    sameAs: (tierAtLeast(supplier.tier, 'pro') || Number(supplier.directory_listing) === 1) && supplier.website_url ? [supplier.website_url] : undefined
  };
  ...
  return {
    localBusiness: safeJsonLd(localBusiness),
    breadcrumb: safeJsonLd(breadcrumb),
    place: safeJsonLd(place)
  };
}
